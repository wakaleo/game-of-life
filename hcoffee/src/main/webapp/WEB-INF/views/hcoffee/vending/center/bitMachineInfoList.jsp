	...
	<script type="text/javascript">
		...
		$(function(){
			...
			$("#searchBut").click(function(){
				$("#pageNo").val(1);
				$("#searchForm").attr("action","${ctx}/hcf/bitMachineInfo/list");
				$("#searchForm").submit();
			});
			...
			$("#exceportBut").click(function(){
				top.$.jBox.confirm("确认要导出数据吗？", "系统提示", function(v, h, f) {
					if (v == "ok") {
						$("#searchForm").attr("action","${ctx}/hcf/bitMachineInfo/exportBase");
						$("#searchForm").submit();
					}
				}, {
					buttonsFocus : 1
			...
			$("#exceportCurrentBut").click(function(){
				top.$.jBox.confirm("确认要导出数据吗？", "系统提示", function(v, h, f) {
					if (v == "ok") {
						$("#searchForm").attr("action","${ctx}/hcf/bitMachineInfo/exportCurrentBase");
						$("#searchForm").submit();
					}
				}, {
					buttonsFocus : 1
				...
				success:function(data){
					...
					if(prizeList != ''){
						...
						/* var imgPreview1 = document.getElementById('preview1'); 
						   imgPreview1.innerHTML = '<img src=${ctxFile}/'+ bitMachineBaseInfoVo.prizeUrl +' />';  */
						/* $("#idWindow").val(bitMachineBaseInfoVo.id); */
...
<body>
	<form:form id="searchForm" modelAttribute="bitMachineBaseInfoVo" action="${ctx}/hcf/bitMachineInfo/list" method="post" class="breadcrumb form-search">
			...
			<tr>
				<%-- <td width="20%" style="text-align:left">
		 		...
		 		<td width="20%" style="text-align:left">
					...
					<form:select path="isOnline" class="input-xlarge required" style="width:120px;"  id="isOnlineId">
						 	<form:option value="" label="全部"/>
						 	<form:option value="1" label="在线"/>
						 	<form:option value="0" label="离线"/>
					</form:select>
				...
				<td width="20%" style="text-align:left">
					<label class="control-label">是否故障：</label>
					<form:select path="isAisleBreak" class="input-xlarge required" style="width:120px;"  id="isAisleBreakId">
						 	<form:option value="" label="全部"/>
						 	<form:option value="1" label="故障"/>
						 	<form:option value="0" label="正常"/>
					</form:select>
				</td>
			 
				<td width="20%" style="text-align:left">
					<label class="control-label">是否断货：</label>
					<form:select path="isOutOfStock" class="input-xlarge required" style="width:120px;"  id="isAisleBreakId">
						 	<form:option value="" label="全部"/>
 				...
 				<td>
 					<form:input path="searchText" htmlEscape="false"  style="width:300px;" class="input-medium" placeholder="支持机器编号、激活码搜索"/>
 				</td>
	...
	<table class="table table-striped table-bordered table-condensed">
		...
		<tbody>
			<c:forEach items="${page.list}" var="model" varStatus="bi">
				<tr>
					<td>${bi.count}</td>
					<td>${model.machineCode}</td>
					<td>${model.id}</td>
					<td>${model.machineName}</td>
					<td>${model.machineMemo}</td>
					<td><fmt:formatDate value="${model.offlineOrOnlineTime}" pattern="yyyy-MM-dd HH:mm:ss"/></td>
					<td>
						<c:choose>
							<c:when test="${model.isOnline == '1'}">
								在线
							</c:when>
							<c:when test="${model.isOnline == '0'}">
								离线
							</c:when>
							<c:otherwise>
								-
					...
					<td>
						<c:choose>
							<c:when test="${model.isAisleBreak == '1'}">
								故障
							</c:when>
							<c:when test="${model.isAisleBreak == '0'}">
								正常
							</c:when>
							<c:otherwise>
								-
					...
					<td><c:choose>
							<c:when test="${model.isOutOfStock == '0'}">
								是
							</c:when>
							<c:when test="${model.isOutOfStock == '1'}">
								否
							</c:when>
							<c:otherwise>
								-
					...
					<td>
					  <%-- <a  href="javascript:void(0)" onclick="showMore('${model.id}')">货道详情</a>&nbsp;&nbsp;&nbsp;&nbsp; --%> 
					  <a  href="${ctx}/hcf/bitMachineInfo/aisleInfoList?machineId=${model.id}"    style=" color:#00F; text-decoration:underline; " >货道详情</a>
					</td>
				</tr>
			</c:forEach>
		</tbody>
