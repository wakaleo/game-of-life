	...
	<script type="text/javascript">
		$(document).ready(function() {
 			$("#btnSubmit").click(function(){
				$("#pageNo").val(1);
				$("#searchForm").attr("action","${ctx}/hcf/activityTemplatet/alwrsLists");
				$("#searchForm").submit();
			});
		...
		$(function(){
			document.onkeydown = function(e){ 
			    ...
			    if(ev.keyCode==13) {
			    	$("#pageNo").val(1);
			    	var actionStr=$("#actionStr").val();
					$("#searchForm").attr("action","${ctx}/hcf/wechatMaterialModel/"+actionStr);
					$("#searchForm").submit();
			     }
			}
		});
		...
		function enable(vendCode,status){
			...
			$.ajax({
				...
				success:function(data){
					alert(data.msg);
		       		if(data.code=="0"){
		       			window.location.href="${ctx}/hcf/activityTemplatet/alwrsLists";
		       		}
...
<body>
	<form:form id="searchForm" modelAttribute="vendingMachineVo" action="${ctx}/hcf/activityTemplatet/alwrsLists" method="get" class="breadcrumb form-search">
		<input id="pageNo" name="pageNo" type="hidden" value="${page.pageNo}"/>
		<input id="pageSize" name="pageSize" type="hidden" value="${page.pageSize}"/>
		<input id="actionStr"  type="hidden" value="${actionStr}"/>
		<input id="getpro"  <c:if test="${actionStr == 'recortList'}">name="wechatModelNo"</c:if> <c:if test="${actionStr == 'revokeList'}">name="oldWechatModelNo"</c:if> type="hidden" value="${wechatModelNo}"/>
		
		<table id="tab" class="table table-striped table-bordered table-condensed" style="width: 100%">
			<label >
					<input type="button" class="btn btn-primary" name="returnSubmit" value="返回模板页面" onclick=" window.history.back();"/>
			</label>
			&nbsp &nbsp &nbsp
			       机器总计： ${pages} 
			<%-- 	<label >
		...
		<tbody>
			<c:forEach items="${page.list}" var="model" varStatus="bi">
				<tr>
					...
					<td><fmt:formatDate value="${model.wechatOptionTime}" pattern="yyyy-MM-dd HH:mm:ss"/></td>
