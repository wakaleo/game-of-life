...
<head>
	...
	$(function(){
		...
		$("#searchBut").click(function(){
			$("#pageNo").val(1);
			$("#searchForm").attr("action","${ctx}/hcf/financialCenter/waitingCloseAccountlist");
			$("#searchForm").submit();
		});
		...
		$("#exportBut").click(function(){
			top.$.jBox.confirm("确认要导出数据吗？","系统提示",function(v,h,f){
				if(v=="ok"){
					$("#searchForm").attr("action","${ctx}/hcf/financialCenter/exportW");
					$("#searchForm").submit();
	...
	//查看详情
	function showInfo_w(amountType,dealerName,checkUpTimes,incomeMoney,loginName){
...
<body>
		<form:form id="searchForm" modelAttribute="waitingAccountVo" action="${ctx}/hcf/financialCenter/waitingCloseAccountlist" method="post" class="breadcrumb form-search">
		...
		<table id="tab" class="table table-striped table-bordered table-condensed" style="width: 100%">
			<tr>
				...
				<label class="control-label">待结算日期：</label> 
				...
				<td  width="15%" style="text-align:left" >
	   				...
	   				<input id="exportBut" class="btn btn-primary" type="button" value="导出excel"/>
				</td>
			</tr>
		</table>
	</form:form>
		...
		<tbody>
			<c:forEach items="${page.list}" var="model" varStatus="bi">
				<tr>
					...
					<td>
						<a href="javascript:void(0)" onclick="showInfo_w(${model.amountType},'${model.dealerName}','${model.checkUpTimes}',${model.incomeMoney},'${model.loginName}')">查看详情</a>
					</td>
				</tr>
			</c:forEach>
		</tbody>
