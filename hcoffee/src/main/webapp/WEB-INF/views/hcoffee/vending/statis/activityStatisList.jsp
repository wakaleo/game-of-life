	...
	<script type="text/javascript">
		$(document).ready(function() {
 			$("#btnSubmit").click(function(){
				$("#pageNo").val(1);
				$("#searchForm").attr("action","${ctx}/hcf/ActivityStatis/list");
				$("#searchForm").submit();
			});
			...
			$("#btnExport").click(function(){
				top.$.jBox.confirm("确认要导出数据吗？","系统提示",function(v,h,f){
					if(v=="ok"){
						...
						$("#searchForm").attr("action","${ctx}/hcf/ActivityStatis/export");
						$("#searchForm").submit();
				...
				top.$.jBox.confirm("确认要导出数据吗？","系统提示",function(v,h,f){
					if(v=="ok"){
						...
						var channelList = new Array();
			           	...
			           	$("#schemeNoList").val(schemeNoList.toString());
			           	$("#channelList").val(channelList.toString());
						$("#searchForm").attr("action","${ctx}/hcf/ActivityStatis/qbexport");
						$("#searchForm").submit();
...
<body>
	<form:form id="searchForm" modelAttribute="activityStatisVo" action="${ctx}/hcf/ActivityStatis/list" method="post" class="breadcrumb form-search">
		...
		<table id="tab" class="table table-striped table-bordered table-condensed" style="width: 100%">
			<tr>
				...
				<td width="8%" style="text-align:left">
					...
					<input name="startTime" type="text"  maxlength="20" class="input-medium Wdate " value="<fmt:formatDate value="${activityStatisVo.startTime}" pattern="yyyy-MM-dd HH:mm:ss"/>"
					onclick="WdatePicker({dateFmt:'yyyy-MM-dd HH:mm:ss',isShowClear:false});" readonly="readonly"/>
					--
					<input name="endTime" type="text"  maxlength="20" class="input-medium Wdate " value="<fmt:formatDate value="${activityStatisVo.endTime}" pattern="yyyy-MM-dd HH:mm:ss"/>"
					onclick="WdatePicker({dateFmt:'yyyy-MM-dd HH:mm:ss',isShowClear:false});" readonly="readonly"/>
		...
		<tbody>
			<c:forEach items="${page.list}" var="model" varStatus="bi">
				<tr>
					...
					<td>
					  <a  href="${ctx}/hcf/ActivityStatis/rewardRecordList?schemeNo=${model.schemeNo}&usrChannel=${model.usrChannel}">详情</a>&nbsp; 
					</td>
				</tr>
			</c:forEach>
		</tbody>
