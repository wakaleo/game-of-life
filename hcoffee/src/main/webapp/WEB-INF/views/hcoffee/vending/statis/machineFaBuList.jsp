...
<head>
	...
	<script type="text/javascript">
		$(document).ready(function() {
 			$("#btnSubmit").click(function(){
				...
				$("#searchForm").attr("action","${ctx}/hcf/activityTemplatet/fabuList?templateId="+templateId);
				$("#searchForm").submit();
			});
		...
		$(function(){
			document.onkeydown = function(e){ 
			    ...
			    if(ev.keyCode==13) {
			    	$("#pageNo").val(1);
					$("#searchForm").attr("action","${ctx}/hcf/activityTemplatet/fabuList");
					$("#searchForm").submit();
			     }
			}
		});
	...
	/* 	function  createTask(vendenCode){
		...
		function enable(vendCode,status){
			...
			$.ajax({
				type:"post",
				url:'${ctx}/hcf/activityTemplatet/enable',
				...
				success:function(data){
					alert(data.msg);
		       		if(data.code=="0"){
		       			window.location.href="${ctx}/hcf/activityTemplatet/fabuList";
		       		}
		...
		function edit(vendCode){
			$.ajax({
				type:"post",
				url:'${ctx}/hcf/activityTemplatet/edit',
				data: vendCode,
				dataType:"json",
				contentType:"application/json",
				success:function(data){
		...
		function record(vendCode){
			$.ajax({
				type:"post",
				url:'${ctx}/hcf/activityTemplatet/upgradeRecord',
				data: vendCode,
				dataType:"json",
				contentType:"application/json",
				success:function(data){
		...
		function batchUpgrade(templateId){
            ...
            if(checkedNum<1){
             ...
             $.ajax({
	                ...
	                data : "vendCodeList="+vendCodeList+"&templateId="+templateId,
					dataType : "json",
		...
		function provinceName(){
			...
			$.ajax({
		       	 type:'post',
		       	 url:'${ctx}/hcf/activityTemplatet/selectProvince',
		       	data: provinceId,
		       	 dataType:"json",
		       	 contentType:"application/json",
		       	 success:function(data){
		...
		function cityName(){
			...
			var  url ='${ctx}/hcf/activityTemplatet/selectCity';
			$.ajax({
		       	 type:'post',
		       	 url:url,
		     	data: cityId,
...
<body>
	<form:form id="searchForm" modelAttribute="vendingMachineVo" action="${ctx}/hcf/activityTemplatet/fabuList" method="post" class="breadcrumb form-search">
				...
				<input id="btnSubmit" class="btn btn-primary" type="button" value="查询" />
				 ...
				 <br>
	   				...
	   				<%-- <label style="padding-right:100px;">
						<a  href="${ctx}/hcf/activityTemplatet/selectBiaoList" style=" font-size:18px;text-decoration:underline; " >收搜售货机地理位置</a>
						</label>  --%>
	...
	<div align="center"> <input class="btn btn-primary" type="button" value="保存" onclick="batchUpgrade('${templateId}')"/></div>
