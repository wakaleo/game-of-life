...
<body>
 <form:form id="recorkeForm" modelAttribute="vendingMachineVo" action="${ctx}/hcf/vendingMachine/list" method="post" class="breadcrumb form-search">
		...
		<table id="tab" class="table table-striped table-bordered table-condensed" style="width: 100%">
			<label >
					<input type="button" class="btn btn-primary" name="returnSubmit" value="返回设备活动管理" onclick=" window.history.back();"/>
			</label>
		</table>
	</form:form> 
		...
		<tbody>
			<c:forEach items="${page.list}" var="model" varStatus="bi">
				<tr>
					...
					<td>
					<c:choose>
							<c:when test="${model.wechatOptionType == 1}">
								更新
							</c:when>
							<c:when test="${model.wechatOptionType == 2}">
								清空
							</c:when>
							<c:otherwise>
								-
